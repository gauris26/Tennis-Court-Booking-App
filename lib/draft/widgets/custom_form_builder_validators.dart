...
class CustomFormBuilderValidators {
  ...
  static FormFieldValidator required({
    ...
    return (valueCandidate) {
      if (valueCandidate == null || ((valueCandidate is Iterable || valueCandidate is String || valueCandidate is Map) && valueCandidate.length == 0)) {
        return errorText;
      }
      return null;
    };
    ...
    return (valueCandidate) {
      if (valueCandidate != null &&
          ((valueCandidate is num && valueCandidate < min) ||
              (valueCandidate is String && num.tryParse(valueCandidate) != null && num.tryParse(valueCandidate) < min))) {
    ...
    return (valueCandidate) {
      if (valueCandidate != null) {
        if ((valueCandidate is num && valueCandidate > max) ||
            (valueCandidate is String && num.tryParse(valueCandidate) != null && num.tryParse(valueCandidate) > max)) {
  ...
  static FormFieldValidator minLength(
    ...
    return (valueCandidate) {
      ...
      if (valueLength < minLength && (!allowEmpty || valueLength > 0)) {
        return errorText ?? 'Value must have a length greater than or equal to $minLength';
      }
      return null;
    };
  }
  ...
  static FormFieldValidator maxLength(
    ...
    return (valueCandidate) {
      if (valueCandidate != null && valueCandidate.length > maxLength) {
        return errorText ?? 'Value must have a length less than or equal to $maxLength';
      }
      return null;
    };
  }
  ...
  static String validateValidators<T>(T val, List<FormFieldValidator<T>> validators) {
